...
class UserController extends Controller
{
    ...
    if(Auth::guard('web')->check()) {
        ...
        if(!is_null($expert)) {
        $jobs = Job::where('country',$user->country)->where('selected',0)->orWhere('role', $user->role)->orWhere('role', $expert->role)->orWhere('country', $expert->country)->get();
        } else {
          $jobs = Job::where('country',$user->country)->where('selected',0)->orWhere('role', $user->role)->get();
        }
    ...
    public function myCv()
    {
        ...
        $levels = Level::all();
        $roles = Role::all();
        $jobs = Job::where('role', $user->role)->where('selected',0)->orWhere('country',$user->country)->get();
        $countries = Country::all();
    ...
    public function store(Request $request)
    {
        switch ($request->select) {
          case 'add_edu':
            ...
            if($sub_special) {
                $edu->ar_sub_special = $sub_special->ar_name;
                $edu->sub_special = $sub_special->name;
            } else {
                $edu->ar_sub_special= $request->sub_special;
                $edu->sub_special = $request->sub_special;
            }
         ...
         case 'lang':
          ...
          default:
            ...
            if($request->has('country')) {
                ...
                if($country) {
                    $expert->ar_country = $country->ar_name;
                    $expert->country = $country->name;
                } else {
                    $expert->ar_country = $request->country;
    ...
    if($request->has('sub_special')) {
            $special = SubSpecial::where('ar_name', $request->sub_special)->orWhere('name' , $request->sub_special)->first();
            if($special) {
                $edu->ar_sub_special = $special->ar_name;
                $edu->sub_special = $special->name;
            } else {
                $edu->ar_sub_special = $request->special;
                $edu->sub_special = $request->special;
            }
            }
    if($edu->save()){
        if(app()->getLocale() == 'ar') {
    ...
    if($request->select == "user_edit") {
       $user = User::findOrFail($id);
        ...
        { 
            ...
            //     $country = Country::where('ar_name' , $request->identity)->first();
            //     if($country) {
            //         $user->ar_country = $country->ar_name;
            //         $user->country = $country->name;
            //     } else {
            ...
            if($request->has('sub_special') && $request->sub_special != "") {
                $special = SubSpecial::where('ar_name' , $request->sub_special)->first();
                if($special) {
                    $user->ar_sub_special = $special->ar_name;
                    $user->sub_special = $special->name;
                } else {
                    $user->ar_sub_special = $request->sub_special;
                    $user->sub_special = $request->sub_special;
                }
            }
            ...
            if($request->has('sub_special') && $request->sub_special !="") {
                $special = SubSpecial::where('name' , $request->sub_special)->first();
                if($special){
                    $user->ar_sub_special = $special->ar_name;
                    $user->sub_special = $special->name;
                } else {
