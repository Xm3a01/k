      ...
      <div class="container">
        <div class="row  pt-5 px-2 mt-4">
            ...
            <div class="bg-white rounded shadow">
              ...
              <ul>
               ...
               <div class="text-center p-3">
                 <a  href="{{route('pdf.download' ,[app()->getLocale() , $user->id])}}" id="download-attachment"
                  class="btn btn-outline-primary px-3  font-weight-bold">{{__('Save as PDF')}} </a>
               </div> 
               <div class="p-3">
                  <hr> <p><span class="text-muted"> {{__('Last updated CV:')}} </span> {{$user->updated_at}}</p>
              </div> 
            </div>
          </div> 
          <div class="col-lg-8 col-md-8 col-sm-12">
              <div class="top1">
                  <!--comlete your cv-->
                  <div class="{{(number_format($count, '0', '.', '') >= 80) ? 'border-success' : 'border-danger' }} my-3 text-center bg-white d-block d-md-flex rounded border-right sec1">
                    <div class="p-3">
                       <span class="{{(number_format($count, '0', '.', '') >= 80) ? 'text-success':'text-danger' }} display-4">{{ number_format($count, '0', '.', '')}}%</span>
                    </div>
                    <p class="align-self-center">{{__('Complete your CV by 80% to be the highlights 10% of the most visible users')}}</p>
                  </div>
                                ...
                                <table class="table table-borderless">
                                   <tr> 
                                    ...
                                    <td><span>{{$expert->end_year - $expert->start_year}}  {{__('Year')}}</span> &  <span>{{abs($expert->end_month - $expert->start_month)}} {{__('Month')}}</span> </td>
                                     ...
                                     <tr> 
                                     <th scope="col">{{__('Job Title')}}</th> 
                                     <td>{{$expert->ar_level}}</td> 
                                  </tr> 
                                  <tr> 
                                     <th scope="col">{{__('Inistitute')}} </th> 
                                     <td><span>{{$expert->company_name}}</span> - <span>{{$user->ar_country}}</span></td>
                                  </tr> 
                                  <tr> 
  ...
  <div class="modal-dialog modal-full" role="document">
      <div class="modal-content fill-cont">
          ...
          <div class="row justify-content-center">
               <form class="form-row col-md-6" action="{{route('users.store', app()->getLocale())}}" method="POST" enctype="multipart/form-data" autocomplete="off">
                    @csrf
                  ...
                  <input type="text" name="ar_name" class="form-control" id="inputAddress"  placeholder="{{__('Arabic name')}}">
                </div>
                <div class="form-group  col-md-6">
                    <label for="password-confirm" class="">  {{ __('Name') }} </label>
                        <input id="password-confirm" type="text" class="form-control" name="name"  placeholder="{{__('Name')}}">
                    </div>
                    
                         <div class="form-group col-md-6">
                      <label for="inputAddress">{{__('Phone')}}</label>
                  <input type="text" name="phone" class="form-control" id="inputAddress"  placeholder="{{__('Phone')}}">
                </div>
                <div class="form-group  col-md-6">
                    <label for="password-confirm" class="">  {{ __('Email') }} </label>
                        <input id="password-confirm" type="email" class="form-control" name="email" autocomplete="new-password" placeholder="{{__('Email')}}">
                    </div>
...
<div class="modal fade" id="addAttch" style="padding-right: 0;" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-full" role="document">
      <div class="modal-content fill-cont">
          ...
          <div class="modal-body p-4" id="result"> 
              <div class="row justify-content-center">
                  <form class="form-row col-md-6" action="{{route('users.store', app()->getLocale())}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="user_id" value = "{{$user->id}}">
                    <input type="hidden" name="select" value = "attch">
                       <div class="form-group col-md-6">
                      <label for="inputAddress">{{__('File Name')}}</label>
                  <input type="text" name="ar_name" class="form-control" id="inputAddress"  placeholder="{{__('Enter Name')}}">
                </div>
                <div class="form-group  col-md-6">
                    <label for="password-confirm" class="">  {{ __('Name by English') }} </label>
                        <input id="password-confirm" type="text" class="form-control" name="name" autocomplete="new-password" placeholder="{{__('Enter Name')}}">
                    </div>
                    <div class="form-group  col-md-12">
                    <label for="password-confirm" class="">  {{ __('Add file') }} </label>
                        <input id="" type="file" class="form-control" name="attch" autocomplete="new-password" placeholder="{{__('')}}">
                    </div>
                  <div class="form-groub col-md-12">
                  <div class="text-center py-5">
                      <button class="btn btn-primary px-3 " type="submit"> {{__('Save')}} </button> 
        ...
        <div class="modal-content fill-cont">
            ...
            <div class="modal-body p-4" id="result"> 
                <div class="row justify-content-center">
                      ...
                      <form class="form-horizontal form-row" id="user-form" role="form" method="POST" action="{{route('users.store' ,app()->getLocale() )}}" enctype="multipart/form-data" autocomplete="off">
                @csrf
                <input type="hidden" name="user_id" value="{{$user->id}}" >
                <input type="hidden" name="select" value="lang" >
                <div class="form-body">
                <div class="row justify-content-center">
                  <div class="form-group col-md-6">
                    ...
                    <div class="">
                        <input  class="form-control"  name="ar_language" placeholder=" {{__('Enter Language')}} ">
                    </div>
                  </div> 
                  <div class="form-group col-md-6">
                    <label class="control-label">{{__('Enter Language by English')}}</label>
                    <div class="">
                        <input  class="form-control"  name="language" placeholder="{{__('Enter Language')}}">
                    </div>
                  </div> 
                  <div class="form-group col-md-12">
            ...
            <div class="row justify-content-center">
                <form method="POST" class="form-row col-md-6" action="{{route('users.store' ,app()->getLocale())}}" enctype="multipart/form-data" autocomplete="off">
                  ...
                  <input type="hidden" name="select" value="add_edu">
                    ...
                    <button class="btn btn-primary btn-outline btn-block" type="submit">{{__('Save')}}</button>
                 </form>
          ...
          <div class="modal-body p-4" id="result"> 
              <div class="row justify-content-center">
                  <form method="POST" class="form-row col-md-6" action="{{route('users.store', app()->getLocale())}}" enctype="multipart/form-data" autocomplete="off">
                    ...
                    <input type="text" class="form-control" name="company_name" id="inputAddress2" placeholder="{{__('Comany Name')}}" >
                      </div>
                      <div class="form-group col-md-3">      
                            <label for="inputEmail4">{{__('Start year')}}</label>
                            <input type="text" class="form-control" name="start_year" id="inputAddress2" placeholder="{{__('Example 2017')}}" >
                      </div>
                      ...
                      <div class="form-group col-md-3">
                        ...
                        <div class="form-group col-md-3">
                          ...
                          <input type="text" class="form-control" id="inputAddress2" placeholder="{{__('Example 2019')}}" name="end_year">
                        </div>
                      ...
                      <div class="form-group col-md-12">
                          ...
                          <textarea class="form-control" id="exampleFormControlTextarea1"
                             placeholder="{{__('Add Job Description')}}"
                      ...
                      <div class="col-md-12 mb-1">
                          <label for="exampleFormControlTextarea1">{{__('Add experience certificate (optional)')}}</label>
                  ...
                  <div class="row justify-content-center">
                        <form class="form-row col-md-6" method="POST" action="{{route('users.update',[app()->getLocale() , $user->id])}}" autocomplete="off">
                          ...
                          <div class="form-group col-md-6">
                            ...
                            <input list="sub_special" name="sub_special" id="inputState" class="form-control" autocomplete="off" placeholder ="{{__('Job title')}}" value="{{(app()->getLocale() == 'ar') ? $user->sub_special : $user->sub_special}}">
                            </div>
                            <datalist id="sub_special" dir="rtl" >
                   ...
                   <input type="hidden" name="select" value="user_edit" id="">
                        ...
                        <div class="form-group col-md-6">
                          ...
                          <div class="form-group col-md-6">
                            ...
                            <input name="email" type="text" class="form-control" id="inputAddress"  value="{{$user->email}}" placeholder=" {{__('Enter Email')}}  ">
                          </div> 
                          <!--<div class="form-group col-md-6">-->
                          <!--    <label for="inputAddress2"> {{__('Current Housing')}}  </label>-->
